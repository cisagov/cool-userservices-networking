...
# cool-userservices-networking Terraform state in the S3 bucket where
...
# cool-userservices-networking state in the Terraform state bucket.
data "aws_iam_policy_document" "read_terraform_state" {
  ...
  statement {
    ...
    resources = [
      "${data.terraform_remote_state.terraform.outputs.state_bucket.arn}/env:/*/cool-userservices-networking/*",
    ]
  }
}

# IAM policy for read-only access to cool-userservices-networking
# Terraform state
